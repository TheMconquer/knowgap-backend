...
@canvas_bp.route('/canvas/test/quiz-questions', methods=['GET'])
async def test_quiz_questions_route():
    """Test route for get_instructor_quiz_questions(). (Temporary - for manual testing)"""
    try:
        auth_header = request.headers.get('Authorization')
        if not auth_header or not auth_header.startswith('Bearer '):
            return jsonify({
        ...
        if not quiz_id or not course_id:
            return jsonify({
                ...
                'message': 'quiz_id and course_id query parameters are required',
                'statusCode': 400
            }), 400

        questions = await get_instructor_quiz_questions(canvas_token, quiz_id, course_id)

        if isinstance(questions, dict) and 'error' in questions:
            return jsonify(questions), questions.get('statusCode', 502)
